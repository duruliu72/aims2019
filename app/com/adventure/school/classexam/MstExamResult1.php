...
class MstExamResult extends Model
{
        ...
        foreach($students as $std){
            ...
            foreach($meargeCourses as $mc){
                ...
                foreach($courses as $course){
                    $group_marks_cats=$this->getGroupMarkCategories($std->programofferid,$std->examnameid,$std->studentid,$course->courseid);
                    // dd($group_marks_cats);
                    ...
                    $marks_cats=$this->getMarkCategories($std->programofferid,$std->examnameid,$std->studentid,$course->courseid);
                    // dd($marks_cats);
    ...
    public function getMearge_Courses($programofferid,$examnameid,$studentid){
        ...
        INNER JOIN student_courses ON mxm.studentid=student_courses.studentid && mxm.courseid=student_courses.courseid
        INNER JOIN courses ON mxm.courseid=courses.id
        WHERE mxm.programofferid=? && mxm.examnameid=? && mxm.studentid=?
        GROUP BY mxm.courseid) AS table1
        INNER JOIN (SELECT
        programofferid,
        courseid,
        ...
        WHERE programofferid=? GROUP BY courseid) AS table2
        ON table1.programofferid=table2.programofferid && table1.courseid=table2.courseid) AS mearge_course
        GROUP BY meargeid";
    ...
    public function getCourses($programofferid,$examnameid,$studentid,$meargeid){
        ...
        FROM `mark_distribution`
        WHERE programofferid=? GROUP BY courseid) AS table2
        ON table1.programofferid=table2.programofferid && table1.courseid=table2.courseid";
        $qResult=\DB::select($sql,[$programofferid,$examnameid,$studentid,$programofferid]);
        $result=collect($qResult);
        // dd($result);
    ...
    public function getGroupMarkCategories($programofferid,$examnameid,$studentid,$coursecodeid){
        ...
        FROM(SELECT
                ...
                INNER JOIN mark_distribution as md on mxm.programofferid=md.programofferid && mxm.courseid=md.courseid && mxm.markcategoryid=md.markcategoryid
                INNER JOIN mark_categories on md.markcategoryid=mark_categories.id
                WHERE mxm.programofferid=? && mxm.examnameid=? && mxm.studentid=? && mxm.courseid=?) AS table1
    ...
    public function getMarkCategories($programofferid,$examnameid,$studentid,$coursecodeid){
        ...
        INNER JOIN mark_distribution as md on mxm.programofferid=md.programofferid && mxm.courseid=md.courseid && mxm.markcategoryid=md.markcategoryid
        INNER JOIN mark_categories on md.markcategoryid=mark_categories.id
        WHERE mxm.programofferid=? && mxm.examnameid=? && mxm.studentid=? && mxm.courseid=?";
        $qResult=\DB::select($sql,[$programofferid,$examnameid,$studentid,$coursecodeid]);
        $result=collect($qResult);
        return $result;
    }
